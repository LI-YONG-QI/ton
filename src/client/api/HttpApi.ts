...
export class HttpApi {
    ...
    this.shardLoader = new DataLoader(async (src) => {
      return await Promise.all(
        src.map(async (v) => {
          ...
          let loaded = (await this.doCall("shards", { seqno: v }, getShards))
            .shards;
  ...
  async callGetMethod(address: Address, method: string, stack: TupleItem[]) {
    return await this.doCall(
      "runGetMethod",
      { address: address.toString(), method, stack: serializeStack(stack) },
      callGetMethod
    );
  }
  ...
  private async doCall<T>(method: string, body: any, codec: z.ZodType<T>) {
    ...
    if (decoded.success) {
      ...
      throw Error(
        "Malformed response: " + decoded.error.format()._errors.join(", ")
      );
    }
  }
}
